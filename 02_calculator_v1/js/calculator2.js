...
function update(calcType) {
  ...
  // １つ前の入力が演算子（演算子 入力しなおし）
    ...
    // 初回入力
    if (wkFirst === "1") {
      element.innerHTML = "0" + calcType; //計算ログ
    }
    else {
      // 演算子 入力しなおし
      let wkLogLastWord = element.innerHTML.slice(-1); //ログ最後の１文字
      if (["+","-","*","/"].includes(wkLogLastWord)) {
        element.innerHTML = element.innerHTML.slice(0, -1) + calcType; //計算ログ　末尾1文字（前回の演算子）を削除
      }
      // イコールの後の演算子
      else{
        element.innerHTML = element.innerHTML + calcType; //計算ログ
      }
    }
  }
